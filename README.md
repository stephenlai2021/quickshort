...
The app is working perfectly on Vercel all the time at the beginning but somehow it is not working anymore due to "Serverless Function crash" or "FUNCTION_INVOCATION_FAILED" but I cannot get it fixed therefore I took alternative approach that is to deploy the project on `Netlify` and it works properly.
